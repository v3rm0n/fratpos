...
interface TransactionRepository : JpaRepository<Transaction, Long> {
    ...
    @Query(
        "SELECT new ee.leola.kassa.models.PopularProduct(tp, sum(tp.quantity)) " +
            "FROM Transaction t " +
            "JOIN t.products tp WHERE t.user = :user AND t.invalid = FALSE " +
            "group by tp.product order by sum(tp.quantity) desc",
    )
    fun findPopularProductsByUser(
